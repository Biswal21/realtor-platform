...
def payment_record_handler(instance: Payment, sender, **kwargs):
    ...
    if pr_status.get("success"):
        ...
        datetime_imojo_modifed_at = datetime.fromisoformat(
            payment_request_imojo.get("modified_at")[:-1]
        ).astimezone(timezone.utc)

        if datetime_imojo_modifed_at > (pr.modified_at):

            # Update payment request
            pr.status = payment_request_imojo.get("status")
            pr.modified_at = datetime_imojo_modifed_at
